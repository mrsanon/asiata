...
Asiata.addCommand({pattern: 'update$', fromMe: true, desc: Lang.UPDATER_DESC, deleteCommand: false}, (async (message, match) => {
    await git.fetch();
...
Asiata.addCommand({pattern: 'update now$', fromMe: true, desc: Lang.UPDATE_NOW_DESC, deleteCommand: false}, (async (message, match) => {
    await git.fetch();
    var commits = await git.log([Config.BRANCH + '..origin/' + Config.BRANCH]);
    if (commits.total === 0) {
