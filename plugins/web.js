/* Copyright (C) 2020 Yusuf Usta. - Copyright (C) 2022 THISAN04
...
Asiata.addCommand({pattern: 'speedtest', fromMe: true, desc: Lang.SPEEDTEST_DESC, deleteCommand: false}, (async (message, match) => {
...
Asiata.addCommand({pattern: 'ping$', fromMe: true, deleteCommand: false, desc: Lang.PING_DESC, deleteCommand: false}, (async (message, match) => {
...
if (Config.WORKTYPE == 'private') {
    ...
    Asiata.addCommand({pattern: 'short ?(.*)', fromMe: true, desc: Lang.URL}, (async (message, match) => {
   ...
   Asiata.addCommand({pattern: 'calc ?(.*)', fromMe: true, desc: Lang.CALC , deleteCommand: false}, (async (message, match) => {
   ...
   Asiata.addCommand({pattern: 'short ?(.*)', fromMe: false, desc: Lang.URL, deleteCommand: false}, (async (message, match) => {
    ...
    Asiata.addCommand({pattern: 'calc ?(.*)', fromMe: false, desc: Lang.CALC , deleteCommand: false}, (async (message, match) => {
        if (match[1].length < 4) { return await message.client.sendMessage(message.jid,Lang.VALİD, MessageType.text) }
        if (match[1].includes('+')) { var split = match[1].split('+'), sonsayi = split[1], ilksayi = split[0]
            var result = -(-ilksayi - sonsayi)
            try { await message.client.sendMessage(message.jid,Lang.SUC + result, MessageType.text) }
